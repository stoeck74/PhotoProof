...
class PhotoProof_Router {
    ...
    public function use_uuid_in_permalink( $post_link, $post ) {
        ...
        // Remplacer uniquement la DERNIÈRE occurrence du slug dans l'URL.
        // (str_replace simple casserait les sites en sous-dossier dont le path
        //  contient le même slug — ex: site sur /test/ avec galerie nommée "test")
        $needle = '/' . $post->post_name . '/';
        $pos    = strrpos( $post_link, $needle );

        if ( false !== $pos ) {
            $post_link = substr_replace( $post_link, '/' . $uuid . '/', $pos, strlen( $needle ) );
        }
...
// L'instanciation est faite une seule fois dans PhotoProof_Plugin::load_dependencies()
