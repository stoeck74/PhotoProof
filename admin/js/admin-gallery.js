...
jQuery(document).ready(function ($) {
    ...
    if (typeof photoproof_vars === 'undefined') {
        console.warn('PhotoProof : photoproof_vars non défini.');
        return;
    }

    var $grid       = $('#pp-gallery-preview');
    var $uploadZone = $('#photoproof_upload_btn');
    var $fileInput  = null;
    var recoIcon    = photoproof_vars.reco_icon || '★';
    var recoEnabled = parseInt(photoproof_vars.reco_enabled, 10) === 1;
    var uploadQueue = [];
    ...
    if (parseInt(photoproof_vars.is_new_post, 10) === 1) {
        $uploadZone.html(
    ...
    $.post(photoproof_vars.ajax_url, {
        action:  'photoproof_get_gallery_photos',
        post_id: photoproof_vars.post_id,
        nonce:   photoproof_vars.nonce
    }, function (response) {
    ...
    function uploadFile(file, callback) {
        ...
        formData.append('post_id', photoproof_vars.post_id);
        formData.append('nonce',   photoproof_vars.nonce);
        formData.append('file',    file);

        $.ajax({
            url:         photoproof_vars.ajax_url,
            type:        'POST',
            data:        formData,
            processData: false,
            contentType: false,
