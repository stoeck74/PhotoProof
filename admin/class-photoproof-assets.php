...
class PhotoProof_Assets {
    ...
    public function enqueue_assets( $hook ) {
        ...
        // Lectures de $_GET en lecture seule pour identifier la page admin
        // (aucune action déclenchée), donc le nonce n'est pas applicable ici.
        ...
        } elseif ( $get_post_id > 0 ) {
            $current_post_type = get_post_type( $get_post_id );
        }
        ...
        if ( $is_settings_page ) {
            ...
            wp_enqueue_script( 'photoproof-animejs' );
            wp_enqueue_script(
                'photoproof-settings-js',
                PHOTOPROOF_URL . 'admin/js/admin-settings.js',
                array( 'jquery', 'photoproof-animejs', 'wp-color-picker' ),
                PHOTOPROOF_VERSION,
                true
            );
        }
        ...
        if ( $is_gallery_page ) {
            wp_enqueue_script( 'photoproof-animejs' );
            wp_enqueue_script(
                'photoproof-gallery-js',
                PHOTOPROOF_URL . 'admin/js/admin-gallery.js',
                array( 'jquery', 'photoproof-animejs' ),
                PHOTOPROOF_VERSION,
                true
            );
            ...
            wp_localize_script( 'photoproof-gallery-js', 'photoproof_vars', array(
