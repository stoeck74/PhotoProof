...
class PhotoProof_Plugin {
    ...
    public function __construct() {
        ...
        // Détection de changement de version pour flush auto des rewrite rules
        add_action( 'init', array( $this, 'maybe_flush_on_version_change' ), 99 );
    }

    /**
     * Si la version stockée en base ne correspond pas à la version du code,
     * on force un flush des rewrite rules (utile lors d'une mise à jour
     * sans désactivation/réactivation manuelle).
     */
    public function maybe_flush_on_version_change() {
        $stored_version = get_option( 'photoproof_version' );

        if ( $stored_version !== PHOTOPROOF_VERSION ) {
            flush_rewrite_rules();
            update_option( 'photoproof_version', PHOTOPROOF_VERSION );
        }
    }

    private function define_constants() {
        define( 'PHOTOPROOF_VERSION', '1.0.0' );
...
$photoproof_instance = new PhotoProof_Plugin();
